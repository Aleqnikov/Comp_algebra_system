...
std::vector<Rational> Validator::validatePolynomial(std::string &number) {
    if (number.empty())
    ...
    result.resize(max_deg, Rational("0/1"));  // Заполняем нулями
    ...
    for (const auto& monom : polynom) {
        ...
        try {
            ...
            result[deg] = Rational(monom.numerator + "/" + monom.denominator);
        } catch (UniversalStringException&) {throw;}
    }
